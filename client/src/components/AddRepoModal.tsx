...
export function AddRepoModal({ onClose }: { onClose: () => void }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setProgress(
        `Loaded ${Object.keys(files).length} files. Building city...`
      );
      ...
      setError(
        err.message || "Failed to fetch repository. Check the URL and try again."
      );
      setIsLoading(false);
    }
  };
  ...
  return (
    <div className={`ho ${visible ? "ho--in" : ""}`} onClick={onClose}>
      <div className="ho-content" onClick={(e) => e.stopPropagation()}>
        ...
        {/* Title */}
        <h2 className="arm-title">Load Repository</h2>
